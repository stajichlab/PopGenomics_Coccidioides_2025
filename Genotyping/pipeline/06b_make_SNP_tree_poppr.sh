...
do
  ...
  do 
    ...
    if [[ ! -s $tree || ${vcf} -nt $tree ]]; then
	    sbatch -p hpcc_default -N 1 -c 8 -n 1 --mem 32gb --out logs/make_poppr_$POPNAME.upgma.%A.log --wrap "time Rscript ./scripts/poppr_tree.R --vcf $vcf --tree $tree --method upgma"
    fi
