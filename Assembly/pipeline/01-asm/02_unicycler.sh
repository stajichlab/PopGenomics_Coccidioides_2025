#!/bin/bash -l
#SBATCH -N 1 -n 1 -c 24 --mem 96gb --out logs/AAFTF_unicycler.%a.log
...
do
    ...
    BASE=$STRAIN
    module load AAFTF
    ASMFILE=$ASM/${ID}.unicycler.fasta
	...
	AAFTF assemble --method unicycler --left $LEFT --right $RIGHT -m $MERGED -c $CPU -o $ASMFILE -w $WORKDIR/unicycler_${ID}

	
	if [ ! -f $ASMFILE ]; then
	    echo "unicycler must have failed, exiting"
	    tail -n 100 $WORKDIR/unicycler_${ID}/unicycler.log
	    exit
	fi
    fi
