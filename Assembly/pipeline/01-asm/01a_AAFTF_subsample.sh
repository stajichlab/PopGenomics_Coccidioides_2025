#!/bin/bash -l
#SBATCH -N 1 -n 1 -c 24 --mem 96gb --out logs/AAFTF_subsample.%a.log
...
do
    ...
    if [ ! -f $LEFT ]; then
		if [ ! -f $LEFTTRIM ]; then
			reformat.sh in=$LEFTIN in2=$RIGHTIN out=$WORKDIR/${ID}_ss_R1.fq.gz out2=$WORKDIR/${ID}_ss_R2.fq.gz samplereadstarget=15000000 
			AAFTF trim --method fastp --dedup --merge --memory $MEM --left $WORKDIR/${ID}_ss_R1.fq.gz --right $WORKDIR/${ID}_ss_R2.fq.gz -c $CPU -o $WORKDIR/${ID}_fastp
